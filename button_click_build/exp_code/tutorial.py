...
def tutorial(win,tutorial_movie):
    ...
    text.text = "In this experiment, you will .......\nPlease use the press the left and right buttons to evaluate the synchrony as it will be shown now\nPress space to start the tutorial"
    text.draw()
    win.flip
    ...
    while mov.status != constants.FINISHED:
        ...
        if (np.round(mov.getCurrentFrameTime(),2) == 6.08) and not high_sync :
            ...
            mov.pause()
            text.text = "This is a scene of high synchrony, so the press repeatedly the right button"
            text.pos = (0,-250)
            win.flip()
        ...
        if (np.round(mov.getCurrentFrameTime(),2) == 8.08) and not low_sync:
            ...
            mov.pause()
            text.text =  "This is a scene of low synchrony, so the press repeatedly the low button"
            text.pos = (0,-250)
            win.flip()
